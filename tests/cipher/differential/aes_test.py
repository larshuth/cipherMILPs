...
class AESTest(unittest.TestCase):
    ...
    def test_correct_actions_performed_round_1_bit_oriented(self):
        ...
        expected_mixcolumns_actions_round_1 = list()

        expected_key_xor_actions_round_1 = list()
        ...
        self.assertEqual(expected_key_xor_actions_pre_round_1, actual_key_xor_actions_test_readable)

        for keyaction in actual_key_xor_actions:
            keyaction.run_action()
        self.K = ['k' + str(cipher_instance.round_number * cipher_instance.key_vars + i) for i in
        ...
        self.assertEqual(round_1_A_before_shift_rows, cipher_instance.A)

        for shiftrowsaction in cipher_instance.generate_shift_rows_actions_for_round():
            shiftrowsaction.run_action()

        self.assertEqual(round_1_A_after_shift_rows, cipher_instance.A)

        for mixcolumnsaction in cipher_instance.generate_mix_columns_actions_for_round():
            mixcolumnsaction.run_action()

        for keyaction in cipher_instance.generate_key_xor_actions_for_round():
            keyaction.run_action()
